...
public class TcpClient implements Runnable {
	...
	/**
	 * When terminate() is called, the TcpSocketWriter and TcpSocketReader
	 * threads will have this parameter as their join() timeout.
	 */
	...
	/**
	 * The instance of TcpSocketWriter associated with this client, which is
	 * used to handle socket writes.
	 */
	private final TcpSocketWriter socketWriter;
	
	/**
	 * The instance of TcpSocketReader associated with this client, which is
	 * used to handle socket reads.
	 */
	 ...
	 * retrieved, this queue is updated with the Message instance describing
	 * the pair. Note that this is a blocking queue which will endlessly grow
	 * if not tended to.
	 * TODO HAVE A LISTENER WHICH PULLS MESSAGES FROM THIS QUEUE
	 */
	private final BlockingQueue<Message> completedMsgQueue;
	
	/**
	 * When this is set to false, if the thread containing this client instance
	 * is interrupted, the thread will die.
	 */
	private boolean keepAlive;
	
	/**
	 * Instantiates a new TcpClient
	 * @param hostname 	The hostname of the server (can be an IP or textual
	 * 						name to be resolved w/ DNS)
	 * @param portNum 	The port number to connect to on the server
	 * @throws UnknownHostException	If the host's IP address couldn't be determined
	 * @throws IOException	If there was an error creating the socket, which can also
	 * 	occur during the instantiation of socketWriter and socketReader
	 */
	 ...
	 * This function then starts the each of the socketWriter and socketReader
	 * threads.
	 */
	public void run() {
		logMsg("entering run()");
	 ...
	 * @throws InterruptedException	If this thread is interrupted when calling join()
	 */
	...
	/**
	 * Stops the TcpClient thread as well as the TcpSocketWriter and
	 * TcpSocketReader threads. The socket is also closed.
	 * This TcpClient should NOT be used after executing this function.
	 * The socket is closed in order to wake the reader thread.
	 * @throws IOException	If an IO error occurs when closing tcpSocket
	 * @throws InterruptedException	If this thread is interrupted when
	 * 	terminating either of the reader or writer threads.
	 */
	...
	/**
	 * Add a completed Message instance (i.e. its response fields are set)
	 * to the completedMsgQueue.
	 * @param completedMsg	A completed Message to add to completedMsgQueue
	 */
	void addToCompletedMsgQueue(Message completedMsg)
	{
		completedMsgQueue.add(completedMsg);
	}
	
	/**
	 * THIS CALL BLOCKS on completedMsgQueue. That is, if this function is
	 * called when completedMsgQueue is empty, the calling thread will wait
	 ...
	 * @throws InterruptedException	If the calling thread is interrupted when
	 * 	waiting on completedMsgQueue.
	 */
	...
	/**
	 * Returns this TcpClient's socketReader member. Intended to be used only
	 * by the socketWriter instance.
	...
	/**
	 * 
	 * @return The default name for the thread executing this class's run()
	 * 	method.
	 */
