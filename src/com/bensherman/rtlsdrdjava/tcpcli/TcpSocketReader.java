...
class TcpSocketReader implements Runnable {
	 ...
	 * @param socket	A socket which is connected to the server
	 * @param tcpClient	The TcpClient which instantiated this TcpSocketReader
	 * @throws IOException	If an error occurs when attempting to call getInputStream() on the socket param
	 */
	TcpSocketReader(Socket socket, TcpClient tcpClient) throws IOException
	{
	...
	/**
	 * Adds the message specified by the parameter to the queue of messages
	 * which are waiting for a response. The add() call is synchronized
	 * using the msgsWaitingForResponseQueue
	 * @param msgNeedingResponse The message whose response is requested
	 */
	void addToWaitingForResponseQueue(Message msgNeedingResponse)
	{
	...
	/**
	 * Executes poll() on msgsWaitingForResponseQueue, which should always
	 * contain at least one Message when this function is executed. After
	 * the Message is pulled from the queue, and the specified parameters
	 * are set.
	 * @param response	The responseMsg field's value to be set
	 * @param responseMsgType	The responseMsgType field's value to be set
	 * @return See description
	 */
	Message pollMostRecentlyRespondedToMsgAndSetResponse(String response, Message.ResponseType responseMsgType)
	{
	 ...
	 * This function will listen to the output from the server, and will
	 * update Message instances created by the TcpSocketWriter thread
	 * with their associated responses. When an response indicating
	 * an update is available or an error occurred, the getResponseFromSocket()
	 * thread will perform an action WHICH MUST BE DEFINED.
	 */
	@Override
	public void run() {
		logMsg("run() entered");
		...
		try
		{
			...
			while (keepAlive)
			{
				try
				{
					Message completedMsg = getResponseFromSocket();
	...
	/**
	 * Each call to this function is intended to build a response for the
	 * oldest message which hasn't been responded to. This function will
	 * build the response message as well as determine its type.
	 * Once the END_OF_RESPONSE specifier has been received, the last
	 * not-responded-to message is retrieved (and removed from the queue),
	 * it's responseMsg and responseMsgType fields are set,and it's returned
	 * to the caller. 
	 * 
	 * The server can return an OK or ERROR response. Also, it can return
	 * an UPDATE_AVAILABLE specifier asynchronously, in the middle of
	 * the current response being generated. In this case, the response string
	 * isn't updated with the UPDATE_AVAILABLE specifier. Instead, the user of
	 * this library MUST define the appropriate action for when such a message
	 * is received. The location where this application-specific change is to
	 * be made is in the comments below. 
	 * 
	 * This function may be terminated due to an IOException being thrown when
	 * the underlying socket is closed.
	 * 
	 * @return The oldest not-responded-to message with set response and response
	 * type fields.
	 * 
	 * @throws IOException 	When an error has occurred when reading the socket.
	 * @throws IllegalStateException	If the new response specifier is sent
	 * 	before the response being built is complete.
	 */
	Message getResponseFromSocket() throws IOException
	{
		...
		while ((readFromSocket = socketBufferedReader.readLine()) != null)
		{
			...
			if (readFromSocket.startsWith(BEGIN_SUCCESSFUL_RESPONSE))
			{
				if (responseBuilder.length() != 0)
				{
					...
					throw new IllegalStateException("Received indicator for a"
							+ " new response before previous response completed!");
				}
				else
				{
		...
		return pollMostRecentlyRespondedToMsgAndSetResponse(
				responseBuilder.toString(), responseMsgType);
	}
	
	/**
	 * Logs/outputs a message. Replace the System.out.println() with a
	 * logger if desired. The message format is "<ThreadName>: msg"
	...
	/**
	 * Sets the keepAlive boolean to false, so that if this thread is
	 * interrupted or the socket is closed, the thread will die.
	 */
